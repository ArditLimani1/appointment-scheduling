...
<head>
    ...
    <title>{{ __('mail.verify_employee.subject', ['business' => $businessName]) }}</title>
</head>
        ...
        <div style="background:#ffffff; border:1px solid #e6e6eb; border-radius:28px; overflow:hidden; box-shadow:0 24px 48px -24px rgba(11, 23, 48, 0.22);">
            <div style="padding:26px 24px 18px; border-bottom:1px solid #efeff3; background:#ffffff;">
                ...
                <div style="margin-top:16px;">
                    <span style="display:inline-block; padding:7px 11px; border:1px solid #e6e6eb; border-radius:999px; background:#fafafc; color:#6b6b78; font-size:11px; font-weight:700; letter-spacing:0.16em; text-transform:uppercase;">{{ __('mail.verify_employee.eyebrow') }}</span>
                </div>
            </div>

            <div style="padding:30px 24px 32px;">
                <h1 style="margin:0 0 14px; font-size:31px; line-height:1.08; letter-spacing:-0.03em; font-weight:800; color:#0a0a0f;">{{ __('mail.verify_employee.title') }}</h1>
                <p style="margin:0 0 18px; font-size:15px; line-height:1.7; color:#3a3a45;">
                    {{ $recipientName
                        ? __('mail.verify_employee.greeting', ['name' => $recipientName, 'business' => $businessName])
                        : __('mail.verify_employee.greeting_generic', ['business' => $businessName]) }}
                </p>
                ...
                <div style="border:1px solid #efeff3; border-radius:22px; background:#fafafc; padding:20px 22px; margin-bottom:24px;">
                    <div style="font-size:12px; font-weight:700; letter-spacing:0.16em; text-transform:uppercase; color:#6b6b78; margin-bottom:14px;">{{ __('mail.verify_employee.next_step') }}</div>
                    <p style="margin:0; font-size:14px; line-height:1.7; color:#3a3a45;">
                        {{ __('mail.verify_employee.next_step_body', ['minutes' => $expiresInMinutes]) }}
                    </p>
                </div>
                ...
                <p style="margin:0; font-size:14px; line-height:1.7; color:#6b6b78;">
                    {{ __('mail.verify_employee.ignore') }}
                </p>
            </div>
        </div>
    </div>
