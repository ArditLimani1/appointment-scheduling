...
const defaultButtonClass = (btnPad) =>
    `relative flex w-full cursor-pointer items-center justify-between gap-2 rounded-xl border border-slate-100 bg-white px-3 ${btnPad} text-left text-sm text-on-surface shadow-sm focus:outline-none focus:ring-2 focus:ring-on-primary-container/20 data-[hover]:border-slate-200`;
...
export default function FilterListbox({
    ...
    const buttonCls = buttonClassName ?? defaultButtonClass(btnPad);
    const panelCls = panelClassName ?? defaultPanelClass;

    return (
        <div className={wrapperClassName ?? `flex-1 ${minWidthClass}`}>
